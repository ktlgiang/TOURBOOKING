...
public class TourWebSocketClient {
            ...
            try {
                ...
                client = new WebSocketClient(new URI(url)) {
                    ...
                    public void onOpen(ServerHandshake handshake) {
                        System.out.println("✅ [TourClient] Connected to " + url);
                        if (listener != null) listener.onOpen();
                        while (!pendingMessages.isEmpty()) {
                            String msg = pendingMessages.poll();
                            client.send(msg);
                            System.out.println("📤 [TourClient] Flushed pending: " + msg);
                        }
                    }

                    @Override
                    public void onMessage(String message) {
                        System.out.println("📩 [TourClient] Received: " + message);
                        if (listener != null) listener.onMessage(message);
                    }

                    @Override
                    public void onClose(int code, String reason, boolean remote) {
                        System.out.println("❌ [TourClient] Closed: " + reason);
                        if (listener != null) listener.onClose(reason);
                    }

                    @Override
                    public void onError(Exception ex) {
                        System.err.println("⚠️ [TourClient] Error: " + ex.getMessage());
    ...
    public static void sendMessage(String msg) {
        if (client != null && client.isOpen()) {
            ...
            System.out.println("⏳ [TourClient] Queued: " + msg);
        }
    }
}
